...
interface VariableSelectorProps {
  ...
  variables: Map<string, boolean> | Map<string, any>; // Can be semafori or realVariables
  editingField: any;
  editingValue: string;
...
export const VariableSelector: React.FC<VariableSelectorProps> = ({
  ...
  const handleVariableSelect = (variableName: string) => {
    // Always use direct editing approach for modular components
    onStartEditing(blockId, field, variableName);
    
    // Immediately save the edit to update the block
    setTimeout(() => {
      onSaveEdit();
    }, 10);
  };
    ...
    <div className="flex items-center space-x-4 w-full">
      <div className="flex items-center space-x-2 flex-1">
        {isEditingVariable ? (
          ...
          <div 
            className={`flex items-center space-x-3 cursor-pointer hover:bg-blue-600/20 px-5 py-3 rounded-xl text-base border flex-shrink-0 w-64 md:w-80 lg:w-96 transition-all transform hover:scale-[1.02] ${
          ...
          >
            <Variable className={`w-6 h-6 ${getActionColor()}`} />
            <div className="flex-1">
            ...
            {/* Current State Indicator - Only for semafori (set/reset actions) */}
