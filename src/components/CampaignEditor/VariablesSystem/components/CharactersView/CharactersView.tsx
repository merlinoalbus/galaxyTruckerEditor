...
export const CharactersView: React.FC<CharactersViewProps> = ({ 
  ...
  const getImageUrl = (character: any) => {
  ...
  const getAllImages = (character: any): { name: string; url: string }[] => {
    ...
    if (character.listaimmagini && Array.isArray(character.listaimmagini)) {
      character.listaimmagini.forEach((img: any) => {
        if (img.binary) {
          // Evita duplicati con l'immagine base
          const isDuplicate = images.some((existing: { name: string; url: string }) => 
